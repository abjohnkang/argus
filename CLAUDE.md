...
Argus is an **on-device AI agent** built on Llama 4. The defining constraint is that
no data ever leaves the device — all inference, memory, and tooling runs locally.
This shapes every architectural decision: cloud APIs, telemetry, and remote model
calls are out of scope.

Authoritative concept document: `CONCEPT.md`. Public-facing summary: `README.md`.

### Current state

The repository is in the **concept / pre-implementation phase**. There is no source
code, no Dockerfile, no `docker-compose.yml`, and no scripts yet. Do not invent
build, lint, or test commands — they do not exist. When asked to "run" or "build",
confirm with the user whether the goal is to *create* the missing scaffolding.

### Planned architecture (from CONCEPT.md)

- **Runtime**: All modules run inside Docker via `docker-compose.yml` so the host
  environment stays untouched.
- **Entry scripts** (to be created):
  - `run_debug.sh` — launch the stack with debug logging visible
  - `run_server.sh` — launch the stack in normal mode
- **First milestone**: a web-based chat UI similar to ChatGPT, backed by a local
  Llama 4 model.

### Working in this repo

- SPECs live in `.moai/specs/` (currently empty). Use `/moai plan "<intent>"` to
  create the first SPEC rather than writing code directly.
- Project brand/design context is in `.moai/project/brand/` and `.moai/design/`.
- The MoAI orchestration rules below govern *how* work gets done; the section
  above governs *what* is being built.
